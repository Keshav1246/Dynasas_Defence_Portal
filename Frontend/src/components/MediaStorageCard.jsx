...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }) => {
  if (percent < 0.05 || percent > 0.95) return null;
...
const MediaStorageCard = ({ mediaBreakdown }) => {
  ...
  const formatStorage = (bytes) => {
    if (bytes === undefined || bytes === 0) return '0.00 KB';
    const k = 1024;
  ...
  const tooltipFormatter = useCallback((val, name, props) => {
    const formatted = props?.bytes !== undefined ? formatStorage(props.bytes) : '0 Bytes';
  ...
  const usedStorageFormatted = formatStorage(totalBytes);
  const usedStorageGB = (totalBytes / (1024 * 1024 * 1024)).toFixed(2);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 flex flex-col h-full">
      <div className="mb-6">
        ...
        <p className="text-[13px] text-gray-500 mt-1">{usedStorageFormatted} / {totalStorageGB} GB used</p>
      </div>

      <div className="flex flex-row items-center justify-between mt-2 flex-grow">
        <div className="w-[45%] h-[180px] min-h-[180px] relative -ml-4">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
            <span className="text-xl font-bold text-gray-900">{usedStorageGB}</span>
        ...
        <div className="w-[55%] flex flex-col justify-center pl-4 space-y-4">
          ...
          <div className="mt-4 pt-4 border-t border-gray-50">
            <div className="flex justify-between items-center mb-1.5">
              ...
              <span className="text-[13px] font-bold text-gray-900">{usedStorageFormatted} / {totalStorageGB} GB</span>
            </div>
            <div className="w-full bg-gray-100 rounded-full h-1.5">
              <div
                className="h-1.5 rounded-full bg-orange-500"
                style={{ width: `${(usedStorageGB / totalStorageGB) * 100}%` }}
              ></div>
            </div>
          </div>
        </div>
